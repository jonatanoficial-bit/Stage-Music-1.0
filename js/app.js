...
(function () {
  ...
  async function fetchOnlineLibrary(){
    const r = await fetch('content/online-library/manifest.json?b=140', {cache:'no-store'});
  ...
  function updateFavoriteButton(){ const s=currentSong(); dom.toggleFavorite.textContent = s && isFavorite(s.id) ? '★' : '☆'; }
  function renderLibrary(){
    const q=dom.searchInput.value.trim().toLowerCase(), genre=dom.genreFilter.value, cls=dom.classificationFilter.value;
  ...
  function updateSetlistStatus(){
    ...
    dom.nextSongCard.textContent=next?`Próxima: ${next.title} — ${next.artist}`:'Nenhuma próxima música';
    const total=state.setlist.length, currentNum=total?Math.min(resolved+1,total):0;
    dom.setlistProgressFill.style.width=total?`${(currentNum/total)*100}%`:'0%';
    dom.setlistProgressLabel.textContent=`${currentNum}/${total}`;
    dom.setlistSummary.textContent=`${total} músicas • ~${minutesEstimate()} min`;
    dom.setlistCount.textContent=`${total} músicas`;
  }
  ...
  function renderCurrentSong(){
    ...
    if(song.pdf){ dom.viewerContent.classList.toggle('mirrored',state.mirrored); dom.viewerContent.innerHTML=`<iframe src="${song.pdf}" style="width:100%;height:72vh;border:none;border-radius:18px;background:#fff"></iframe>`; dom.metaKey.textContent=song.key||'—'; return; }
  ...
  function openSong(id){
    ...
    dom.metaBpm.textContent=song.bpm||'—'; dom.metaCapo.textContent=song.capo||'—'; dom.metaSource.textContent=song.source||'—';
  ...
  async function loadLibrary(){
    try{
      ...
      dom.libraryCount.textContent='erro';
  ...
  function setViewerSize(size){ document.documentElement.style.setProperty('--size',`${size}px`); dom.fontRange.value=size; }
  ...
  function jumpToTrack(){ const idx=Number(dom.jumpInput.value||0)-1; if(idx>=0&&idx<state.setlist.length) openSong(state.setlist[idx].id); }
  function saveSetlist(){ const name=prompt('Nome do repertório:', `Setlist ${new Date().toLocaleDateString('pt-BR')}`); if(!name) return; state.savedSetlists.unshift({id:`setlist-${Date.now()}`,name,createdAt:new Date().toISOString(),songs:state.setlist}); writeSavedSetlists(state.savedSetlists); dom.setlistTitle.textContent=`Setlist atual • ${name}`; renderSavedPlaylists(); }
  function loadSavedSetlist(id){ const item=state.savedSetlists.find(x=>x.id===id); if(!item) return; state.setlist=(item.songs||[]).map(song=>({...song})); dom.setlistTitle.textContent=`Setlist atual • ${item.name}`; renderSetlist(); if(state.setlist.length) openSong(state.setlist[0].id); }
  ...
  function bindSwipe(){ dom.viewerCard.addEventListener('touchstart',e=>{ state.touchStartX=e.changedTouches[0].clientX; },{passive:true}); dom.viewerCard.addEventListener('touchend',e=>{ const deltaX=e.changedTouches[0].clientX-state.touchStartX; if(Math.abs(deltaX)<50) return; if(deltaX<0) goRelative(1); else goRelative(-1); },{passive:true}); }
  ...
  dom.classificationFilter.addEventListener('change',renderLibrary);
  dom.refreshLibrary.addEventListener('click',loadLibrary);
  dom.pdfInput.addEventListener('change',handlePdfUpload);
  dom.toggleStageMode.addEventListener('click',()=>document.body.classList.toggle('stage-mode'));
  dom.increaseFont.addEventListener('click',()=>setViewerSize(Math.min(48,Number(dom.fontRange.value||22)+1)));
  dom.decreaseFont.addEventListener('click',()=>setViewerSize(Math.max(14,Number(dom.fontRange.value||22)-1)));
  dom.fontRange.addEventListener('input',e=>setViewerSize(e.target.value));
  ...
  dom.addToSetlist.addEventListener('click',addCurrentToSetlist);
  dom.nextSong.addEventListener('click',()=>goRelative(1));
  dom.prevSong.addEventListener('click',()=>goRelative(-1));
  dom.jumpButton.addEventListener('click',jumpToTrack);
  dom.jumpInput.addEventListener('keydown',e=>{ if(e.key==='Enter') jumpToTrack(); });
  dom.saveSetlist.addEventListener('click',saveSetlist);
  dom.clearSetlist.addEventListener('click',()=>{ state.setlist=[]; renderSetlist(); });
  dom.copyText.addEventListener('click',async()=>{ const song=currentSong(); if(!song||song.pdf) return; try{ await navigator.clipboard.writeText(transposeText(song.lyrics||'',state.transpose)); dom.copyText.textContent='Copiado'; setTimeout(()=>dom.copyText.textContent='Copiar',1200); }catch(e){} });
  dom.toggleFavorite.addEventListener('click',()=>{ const song=currentSong(); if(!song) return; if(isFavorite(song.id)) state.favorites=state.favorites.filter(x=>x.id!==song.id); else state.favorites.unshift({id:song.id,title:song.title}); writeFavorites(state.favorites); renderLibrary(); renderSetlist(); updateFavoriteButton(); });
  dom.transposeUp.addEventListener('click',()=>{ state.transpose+=1; renderCurrentSong(); });
  dom.transposeDown.addEventListener('click',()=>{ state.transpose-=1; renderCurrentSong(); });
  dom.toggleMirror.addEventListener('click',()=>{ state.mirrored=!state.mirrored; renderCurrentSong(); });
